...
* **어댑터 타입 2: Semantic Layer (Cube.js) - 내부 통합**
  * Text-to-SQL의 환각을 방지하기 위해 AI는 사전 정의된 Dimension/Measure JSON만 생성하고, 어댑터가 이를 Cube 쿼리로 변환.
  * **내부 데이터소스 관리:** UMG가 Cube.js에 연결될 DB를 직접 관리. DB 연결 정보 암호화 저장, 연결 테스트, 6종 DB 지원.
  * **내부 스키마 관리:** Cube.js 모델(measures, dimensions, joins)을 UMG 내부에서 CRUD. 버전 관리 및 검증 포함.
  * **메타 API:** 활성 큐브/measure/dimension 메타정보 조회, DB 테이블 인트로스펙션.
  * **자동 도구 생성:** Cube.js 스키마로부터 MCP 도구를 자동 생성하여 AI Agent가 즉시 사용 가능.
* **어댑터 타입 3: Remote MCP Proxy (AWS MCP 연동) - 내부 통합**
  * AWS 환경(VPC 내)에 배포된 원격 MCP 서버들로 요청을 포워딩.
  * UMG가 AWS IAM 역할을 부여받아 **AWS SigV4 인증 서명**을 생성하여 원격 서버에 안전하게 접속.
  * **서버 레지스트리:** AWS MCP 서버를 UMG에 등록/관리. 엔드포인트, 리전, 인증 정보를 중앙 관리.
  * **연결 테스트:** SigV4 서명으로 실제 서버에 initialize 요청을 보내 연결 상태 확인.
  * **도구 자동 동기화:** 등록된 서버에 tools/list를 호출하여 사용 가능한 도구를 자동 발견 및 등록.
  * **STS AssumeRole:** IAM Role ARN 기반 임시 자격 증명으로 보안 강화.
...
### **Phase 7: Cube.js 내부 통합 및 AWS MCP 서버 관리 (Internal Integration)**
> Cube.js와 AWS MCP를 단순 외부 호출이 아닌 **내부 포함(Built-in) 기능**으로 격상.
> 데이터소스 관리, 스키마 관리, 연결 테스트, 서버 레지스트리, 도구 자동 동기화를 포함.

#### Phase 7-A: Cube.js 시맨틱 레이어 내부 통합
* **데이터소스(DataSource) 관리:**
  - `CUBE_DATA_SOURCES` 테이블: Cube.js가 연결할 DB 정보(PostgreSQL, MySQL, BigQuery 등)를 UMG가 중앙 관리.
  - 데이터소스 CRUD API (`/api/cube/datasources`): 생성, 수정, 삭제, 연결 테스트.
  - DB 연결 정보(호스트, 포트, DB명, 사용자, 비밀번호)는 AES-256 암호화 저장.
  - 연결 테스트(Connection Test) 기능으로 등록 전 실제 DB 연결 확인.
  - 지원 DB 타입: `POSTGRESQL`, `MYSQL`, `BIGQUERY`, `REDSHIFT`, `SNOWFLAKE`, `CLICKHOUSE`.
* **Cube.js 스키마(모델) 관리:**
  - `CUBE_SCHEMAS` 테이블: Cube.js 모델(measures, dimensions, joins) 정의를 DB에 저장.
  - 스키마 CRUD API (`/api/cube/schemas`): YAML/JSON 형식 모델 생성, 수정, 삭제, 버전 관리.
  - 스키마 검증(Validation) 기능: measures/dimensions 구조 검증.
  - 스키마를 데이터소스에 연결하여 실제 Cube.js 쿼리 실행 시 자동 적용.
  - 스키마 버전 이력(version) 관리로 롤백 지원.
* **Cube.js 쿼리 메타 API:**
  - `/api/cube/meta` 엔드포인트: 현재 활성화된 cubes, measures, dimensions 메타정보 조회.
  - 데이터소스별 사용 가능한 테이블/컬럼 스캔(Introspection) 기능.
  - 스키마 기반 자동 도구 생성: Cube.js 스키마로부터 MCP 도구를 자동으로 생성.
* **Cube.js 어댑터 강화:**
  - 기존 `CubeJsAdapter`를 `CubeDataSource` + `CubeSchema`와 통합.
  - 도구의 connectionConfig 대신 내부 데이터소스 ID로 연결 관리.
  - pre-aggregation 힌트 지원, 쿼리 실행 계획(explain) 기능.
  - 쿼리 결과 캐싱 (Redis).

#### Phase 7-B: AWS MCP 서버 레지스트리 및 관리
* **AWS MCP 서버 레지스트리:**
  - `AWS_MCP_SERVERS` 테이블: AWS 원격 MCP 서버 정보를 중앙 관리.
  - 서버 CRUD API (`/api/aws-mcp/servers`): 등록, 수정, 삭제.
  - AWS 자격 증명(IAM Access Key, Secret Key, Role ARN)은 AES-256 암호화 저장.
  - 서버 상태(ACTIVE, INACTIVE, ERROR) 관리.
  - 지원 AWS 서비스: Redshift, S3 Tables, CloudWatch, Cost Analysis, SOPS 등.
* **연결 관리:**
  - 연결 테스트(Connection Test): SigV4 서명으로 실제 MCP 서버에 `initialize` 요청 전송.
  - 서버 상태 모니터링: 주기적 헬스체크 (선택적).
  - STS AssumeRole 지원: IAM Role ARN을 통한 임시 자격 증명 발급 지원.
* **도구 자동 동기화:**
  - 등록된 AWS MCP 서버에 `tools/list` 호출하여 사용 가능한 도구 자동 검색.
  - 검색된 도구를 UMG의 TOOLS 테이블에 자동 등록 (PENDING 상태).
  - 동기화 이력(Sync History) 기록.
  - 수동/자동 동기화 트리거 API (`/api/aws-mcp/servers/{id}/sync`).
* **AWS 어댑터 강화:**
  - 기존 `AwsRemoteMcpProxyAdapter`를 `AwsMcpServer` 레지스트리와 통합.
  - 도구의 connectionConfig 대신 내부 서버 ID로 연결 관리.
  - STS 임시 자격 증명 자동 갱신 및 캐싱.
  - 요청별 자격 증명 주입으로 보안 강화.

#### Phase 7-C: 프론트엔드 관리 UI
* **Cube.js 관리 페이지 (`/cube`):**
  - 데이터소스 관리 탭: 데이터소스 목록, 추가/수정/삭제, 연결 테스트 버튼.
  - 스키마 관리 탭: 모델 에디터(코드 에디터), 검증, 버전 이력.
  - 메타 탐색기 탭: 큐브/measure/dimension 탐색, 테이블 스캔 결과.
* **AWS MCP 관리 페이지 (`/aws-mcp`):**
  - 서버 목록: 상태 표시, 마지막 동기화 시간, 도구 수.
  - 서버 등록/수정 폼: 엔드포인트, 리전, 인증 방식 설정.
  - 도구 동기화 버튼 및 동기화 이력 뷰.
...
erDiagram
    ...
    CUBE_DATA_SOURCES {
        UUID id PK
        String name UK "데이터소스 고유 이름"
        String description
        String db_type "POSTGRESQL/MYSQL/BIGQUERY/REDSHIFT/SNOWFLAKE/CLICKHOUSE"
        TEXT connection_config "AES-256 암호화 (host,port,db,user,pass)"
        String status "ACTIVE/INACTIVE/ERROR"
        TimestampTZ last_tested_at "마지막 연결 테스트 시각"
        UUID created_by FK
        TimestampTZ created_at
        TimestampTZ updated_at
    }
    CUBE_SCHEMAS {
        ...
        String name UK "스키마(모델) 이름"
        String description
        TEXT schema_definition "YAML/JSON Cube.js 모델 정의"
        Integer version "스키마 버전 번호"
    ...
    AWS_MCP_SERVERS {
        UUID id PK
        String name UK "서버 고유 이름"
        String description
        String endpoint_url "MCP 서버 엔드포인트"
        String region "AWS 리전"
        String service "AWS 서비스명 (기본: execute-api)"
        String auth_type "IAM_KEY/IAM_ROLE"
        TEXT credentials_config "AES-256 암호화 (accessKey,secretKey,roleArn)"
        String status "ACTIVE/INACTIVE/ERROR"
        Integer synced_tool_count "동기화된 도구 수"
        TimestampTZ last_synced_at "마지막 동기화 시각"
        TimestampTZ last_health_check_at "마지막 헬스체크 시각"
        UUID created_by FK
        TimestampTZ created_at
        TimestampTZ updated_at
    }
    AWS_MCP_SYNC_HISTORY {
        UUID id PK
        UUID server_id FK
        String status "SUCCESS/FAIL"
        Integer tools_discovered "발견된 도구 수"
        Integer tools_created "신규 등록된 도구 수"
        Integer tools_updated "업데이트된 도구 수"
        TEXT error_message
        TimestampTZ created_at
    }
    ...
    USERS ||--o{ CUBE_DATA_SOURCES : "생성"
    CUBE_DATA_SOURCES ||--o{ CUBE_SCHEMAS : "포함"
    USERS ||--o{ CUBE_SCHEMAS : "생성"
    USERS ||--o{ AWS_MCP_SERVERS : "등록"
    AWS_MCP_SERVERS ||--o{ AWS_MCP_SYNC_HISTORY : "동기화 이력"
    AWS_MCP_SERVERS ||--o{ TOOLS : "동기화된 도구"
```
...
|   |   +-- cube/               Cube.js 내부 통합 (데이터소스, 스키마 관리)
|   |   +-- aws/                AWS MCP 서버 레지스트리 및 동기화
|   |   +-- mcp/                MCP 프로토콜 핸들러
